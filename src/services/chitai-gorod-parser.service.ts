...
export class ChitaiGorodParserService {
    ...
    async parseChitaiGorodBook(): Promise<Books[]> {
        ...
        for (let page = 1; page <= maxPages; page++) {
            const responce = await axios.get(
                `https://www.chitai-gorod.ru/catalog/books/hudozhestvennaya-literatura-110001?page=${page}`
            );
            ...
            if (responce.status === 200) {
                ...
                const promises = $('.products-list .product-card').map(
                    async (index, element) => {
                        ...
                        if (!existingBook) {
                            ...
                            await this.parseAdditionalInfoBook(
                                newBook.id,
                                `https://www.chitai-gorod.ru${bookUrl}`,
                            );
    ...
    async parseMaxPages(): Promise<number> {
        ...
        if (responce.status === 200) {
            ...
            const maxPages = parseInt(pagintationElement, 10);
